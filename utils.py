...
import os
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), './ViT_models'))

os.environ['CUDA_LAUNCH_BLOCKING'] = "1"
os.environ["CUBLAS_LOG_LEVEL"] = "4"
...
def load_model(configs):
    ...
    if configs.model in ('inception_v3', 'googlenet'):
        model = models.__dict__[configs.model](pretrained=True, aux_logits=True).cuda()
    else:
...
def load_model_group4_norm(configs):
    ...
    if configs.model == 'deit_tiny':
        model = __import__('deit', fromlist=['*']).deit_tiny(pretrained=True).cuda()
    elif configs.model == 'deit_small':
        model = __import__('deit', fromlist=['*']).deit_small(pretrained=True).cuda()
    elif configs.model == 'deit_base':
        model = __import__('deit', fromlist=['*']).deit_base(pretrained=True).cuda()
    elif configs.model == 'dino_small':
        model = __import__('deit', fromlist=['*']).dino_small(pretrained=True).cuda()
        max_mem = 10 * 1024 ** 3
        torch.cuda.max_memory_allocated = max_mem
        print("Memory Usage:",
              torch.cuda.memory_allocated() / max_mem)
        model = model.eval()
        print("GPU Memory Usage:")
        print(torch.cuda.memory_allocated() / max_mem, "percentage")
    elif configs.model == 'dino_base':
        model = __import__('deit', fromlist=['*']).dino_base(pretrained=True).cuda()
    elif configs.model == 'mocov3_small':
        model = __import__('mocov3', fromlist=['*']).mocov3_small(pretrained=True).cuda()
    elif configs.model == 'pvt_tiny':
        model = __import__('pvt', fromlist=['*']).pvt_tiny(pretrained=True).cuda()
    elif configs.model == 'pvt_small':
        model = __import__('pvt', fromlist=['*']).pvt_small(pretrained=True).cuda()
    elif configs.model == 'pvt_medium':
        model = __import__('pvt', fromlist=['*']).pvt_medium(pretrained=True).cuda()
    elif configs.model == 'pvtv2_b2':
        model = __import__('pvt_v2', fromlist=['*']).pvtv2_b2(pretrained=True).cuda()
    elif configs.model == 'pvtv2_b3':
        model = __import__('pvt_v2', fromlist=['*']).pvtv2_b3(pretrained=True).cuda()
    elif configs.model == 'swin_t':
        model = __import__('swin', fromlist=['*']).swin_t(pretrained=True).cuda()
    elif configs.model == 'swin_s':
        model = __import__('swin', fromlist=['*']).swin_s(pretrained=True).cuda()

    else:
        model = models.__dict__[configs.model](pretrained=True).cuda()

    if configs.model in ['swin_t', 'swin_s', 'swin_b', 'pvt_tiny', 'pvt_small', 'pvt_medium', 'deit_tiny', 'deit_small',
                         'deit_base', 'dino_small', 'dino_base', 'mocov3_small']:
        fc_layer = model.norm
