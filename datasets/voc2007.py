...
def read_object_labels(root, dataset, set):
    ...
    #path_labels = '/mnt/petrelfs/mengfanqing/SFDA-main/data/VOC2007/meta/Main/'
    labeled_data = dict()
...
class VOC2007(data.Dataset):
    ...
    def __init__(self, root, split, transform=None, target_transform=None, download=False):
        ...
        if not os.path.exists(file_csv):
            ...
            # generate csv file
            labeled_data = read_object_labels(self.root, 'VOC2007', self.split)
            # write csv file
        ...
        print('[dataset] VOC 2007 classification set=%s number of classes=%d  number of images=%d' % (
            self.split, len(self.classes), len(self.images)))

    def __getitem__(self, index):
        path, target = self.images[index]
        target = np.array(target == 1, dtype=int)
