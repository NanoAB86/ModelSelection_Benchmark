...
from metrics import  LFC, NCE1, LEEP, LEEPvit, gbc, NCE, NLEEP, LogME_Score, PAC_Score, SFDA_Score, PARC_Score, EMMS, Transrate, LDA_Score, \
    Energy_Score, Hscore, load_score, NCTI_Score
#from metricsTest import EMMS,  Energy_Score, EMMS_optimal, stacked_score
...
if __name__ == "__main__":
    ...
    parser.add_argument('-me', '--metric', type=str, default='NewEMMS',
    ...
    for model in models_hub:
        ...
        # y_labels = y_labels2
        #print(y_labels_0.shape)
        #print(y_labels_1.shape)
        #print(y_labels_2.shape)
        y_labels1 = np.stack((y_labels_0, y_labels_1, y_labels_2), axis=2)

        #leep, logme, nleep, emms, transrate, energy, pac, nce, gbc, lda, hscore, parc, lfc
        args.metric = 'emms'
        if args.metric == 'emms':
            score_dict[args.model] = EMMS(X_features, y_labels1)
        elif args.metric == 'leep':
            score_dict[args.model] = LEEPvit(X_features, y_labels)
        elif args.metric == 'lfc':
            score_dict[args.model] = LFC(X_features, y_labels)
            ...
            # Compute OT-based NCE score
            P, _ = compute_coupling(X_features, target_features)  # Compute coupling
            score_dict[args.model] = -compute_CE(P, y_labels, target_y_labels)  # Use negat
        ...
        elif args.metric == 'leepvit':
            score_dict[args.model] = LEEPvit(X_features, y_labels, model_name=args.model)
        elif args.metric == 'lda':
            score_dict[args.model] = LDA_Score(X_features, y_labels)
        elif args.metric == 'energy':
            score_dict[args.model] = Energy_Score(X_features, 0.5, 'bot').tolist()
        elif args.metric == 'pac':
            pg, _ = PAC_Score(X_features, y_labels, lda_factor=1)
            score_dict[args.model] = -pg[0][1]
        elif args.metric == 'nce':
            score_dict[args.model] = NCE(X_features[0], y_labels)
        elif args.metric == 'lfc':
            score_dict[args.model] = LFC(X_features, y_labels)
        elif args.metric == 'hscore':
            score_dict[args.model] = Hscore(X_features, y_labels)
        elif args.metric == 'gbc':
            score_dict[args.model] = gbc(X_features, y_labels, 'diagonal')
